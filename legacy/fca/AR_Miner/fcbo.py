...
if __name__ == "__main__":
    ...
    if len(sys.argv)<6:
        ...
        print (_filepath)
        c=Context(_filepath,'context');#'./csv/csvspec.json'
       # c.writeObjects(str(sys.argv[2]))#'objects.txt')
       # c.writeAttributes(str(sys.argv[3]))#'Attributes.txt')
        #c.writeContext2Fimi(str(sys.argv[4]))
       # c.writeContext2Rcf(str(sys.argv[5]))
        t="[Relational Context]"+'\n'
        d="Default Name"+'\n'
        b="[Binary Relation]"+'\n'
        n="Name_of_dataset"+'\n'
        e="[END Relational Context]"+'\n'
        fullcontext=[' '.join(e)  for e in c.context]+['\n']
        objs=[' | '.join([str(obj) for obj in c.objects])]+['\n']
        atts=[' | '.join([str(att) for att in c.attributes])]+['\n']
        filecontent2=['\n'.join([''.join([str(i) for i in v]) for v in fullcontext])]+["[END Relational Context]"+'\n']
        filecontent=''.join([t]+[d]+[b]+[n]+objs+atts+filecontent2 )
