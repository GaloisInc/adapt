...
def createVertices(graph,g,criterion) {
  ...
  existingSegNodes_parentIds = g.V().has('segment:name','byPID').values('segment:parentId').fold().next();
...
def createVerticesAndEdges(graph,g,criterion,radius) {
  ...
  for (i in idWithProp) {
    ...
    if (i in idsToStore) { 
      ...
      s = g.V().has('segment:name','byPID').has('segment:parentId',i).next()
    }; 
    idNonLinkedNodes = subtr.V().id().fold().next()- g.V().has('segment:name','byPID').has('segment:parentId',i).outE('segment:includes').inV().id().fold().next();
