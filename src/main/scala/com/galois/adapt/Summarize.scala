    ...
    def sendQueryFileActivities(): Future[List[ProcessFileActivity]] = {
      ...
      val fileQuery =
        s"""g.V().hasLabel('AdmPathNode').has('path', '$processPath').as('processPaths')
           ...
           |.select('subject')
           |.in('subject').as('eventNode').values('earliestTimestampNanos').as('earliestTimestampNanos')
           |.select('eventNode').values('eventType').as('eventType')
           |.select('eventNode')
           |.out('predicateObject2', 'predicateObject').hasLabel('AdmFileObject').as('fileObjects')
           |.values('fileObjectType').as('fot')
           |.select('fileObjects')
           |.out('path', '(path)').values('path').as('filePath')
