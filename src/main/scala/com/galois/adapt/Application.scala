...
//    new File(config.getString("adapt.runtime.iforestpath")).setExecutable(true)
  ...
  val fileDb = Try { config.getString("adapt.adm.mapdb") } match {
    case Success(p) =>
      ...
      if (config.getBoolean("adapt.adm.mapdb.mapdbbypasschecksum")) maker = maker.checksumHeaderBypass()
      if (config.getBoolean("adapt.adm.mapdb.mapdbtransactions")) maker = maker.transactionEnable()
