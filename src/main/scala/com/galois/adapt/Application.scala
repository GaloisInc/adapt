...
import com.bbn.tc.services.kafka.NewCDMConsumer
...
//    dbFile.delete()
  ...
  runFlow match {
    ...
    case "ssltest" =>
      println("Running Kafka SSL test")
      CDMSource.cdm18(ta1).runWith(Sink.foreach(d => println(d._2)))  // tested. works.
...
//  def kafkaSslSource[C](ta1Topic: String): Source[C, _] = {
...
//    val kafkaConsumer = new KafkaSslConsumer()
//    kafkaConsumer.start()
...
class KafkaSslConsumer(
  kafkaServer: String = Application.config.getString("akka.kafka.consumer.kafka-clients.bootstrap.servers"),
  groupId: String = Application.config.getString("akka.kafka.consumer.kafka-clients.group.id"),
  topics: String = Application.config.getString("adapt.env.ta1kafkatopic"),
  duration: Int = 0, // No records after this many seconds results in exiting. Initial duration is 5x this number. 0 means no limit (maybe...?)
  consumeAll: Boolean = true,
  isSpecific: Boolean = true,
  consumerSchemaFilename: String = Application.config.getString("adapt.env.cdmavroschemafilepath"),
  producerSchemaFilename: String = Application.config.getString("adapt.env.cdmavroschemafilepath"),
  autoOffset: String = Application.config.getString("adapt.env.autooffset"),
  specificOffset: String = Application.config.getString("adapt.ingest.startatoffset"), // This *needs* to be either null or else a string that can be parsed as a Long, which will stop consuming after that many records are ingested.
  noavro: Boolean = false,
  maxRecords: Long = -1L,
  recordSize: Int = -1,
  file: File = null,
  securityProtocol: String = "SSL", //Application.config.getString("adapt.env.cdmavroschemafilepath"),
  truststoreLocation: String = Application.config.getString("adapt.env.truststorepath"),
  truststorePassword: String = Application.config.getString("adapt.env.trustpass"),
  keystoreLocation: String = Application.config.getString("adapt.env.keystorepath"),
  keystorePassword: String = Application.config.getString("adapt.env.keypass"),
  sslKey: String = Application.config.getString("adapt.env.sslkey")
) extends NewCDMConsumer(kafkaServer, groupId, topics, duration, consumeAll, isSpecific, consumerSchemaFilename,
  producerSchemaFilename, autoOffset, specificOffset, noavro, maxRecords, recordSize, file, securityProtocol, truststoreLocation, truststorePassword,
  keystoreLocation, keystorePassword, sslKey
) {
  override def processCompiledRecord(key: String, datum: TCCDMDatum) = {
    println(s"Key: $key, datum: $datum")
    super.processCompiledRecord(key, datum)
  }

  override def getConfig = super.getConfig

  println(s"Instantiated with: ${(
    kafkaServer, groupId, topics, duration, consumeAll, isSpecific, consumerSchemaFilename,
