...
//      getFromResourceDirectory("web")   // Serve what is packaged (e.g. into a fat jar) from the resource directory in the jar
  ...
  def mainRoute(
       ...
       ppmActors: Map[HostName, PpmManager]
          ...
          pathPrefix("ppm") {
            pathPrefix("saveTrees") {
              path(Segment) { hostName =>
                complete(
                  ppmActors(hostName).saveTrees().map(_ => s"you saved all the tree reprs and their alarms for $hostName")
                )
              } ~
              complete(
                ppmActors.values.toList.foldLeft(Future.successful( "No trees to save" ))((a, b) => a.flatMap(_ => b.saveTrees()).map(_ => "you saved all the trees for all the hosts"))
              )
            } ~
            pathPrefix("listTrees") {
              path(Segment) { hostName =>
                complete(
                  ppmActors(hostName).ppmList.map(_.treeName).toList  // ? ListPpmTrees).mapTo[Future[PpmTreeNames]].flatMap(_.map(_.namesAndCounts))
                )
              } ~
              complete(
                ppmActors.mapValues(_.ppmList.map(_.treeName).toList)
//                Future.sequence(
