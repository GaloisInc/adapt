...
case class Value(
  ...
  val asDBEdges: List[(CDM19.EdgeTypes.EdgeTypes,UUID)] = tagsFolded.map(t => (CDM19.EdgeTypes.tag,t.getUuid))

  override val supportNodes = tagsFolded.flatMap(t => (t.getUuid, t.asDBKeyValues, t.asDBEdges) :: t.supportNodes)
  
}

case object Value extends CDM19Constructor[Value] {
  type RawCDMType = cdm19.Value

  def from(cdm: RawCDM19Type): Try[Value] = Try(
    Value(
      cdm.getSize,
      cdm.getType,
