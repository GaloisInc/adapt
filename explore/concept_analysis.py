...
def printRuleViolations(vios,context,rules,type_rule,printing_flag=True,namedentities=False,output=sys.stdout):
    out=openFile(output)
    sep_line='------------------------------------------'
    printlist=[]
    for v in vios:
...
def printComputedRules(rule_gen_results,context,output=sys.stdout):
